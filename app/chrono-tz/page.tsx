...
const CHRONO_VERSION = (pkg.dependencies as Record<string, string>)[
...
function ConstructionLab() {
	...
	const fromFmt = DateTime.fromFormat(
		"25/12/2024 8:00 PM",
		"DD/MM/YYYY h:mm A",
	...
	const fromDate = DateTime.fromDate(
		new Date("2024-03-14T15:09:26"),
		"Europe/Berlin",
	);

	const examples = [
		{
			m: `DateTime.now('America/New_York')`,
			v: now.format("YYYY-MM-DD HH:mm:ss Z"),
		},
		{ m: `DateTime.nowUTC()`, v: DateTime.nowUTC().toISO() },
		{
			m: `DateTime.fromISO('2024-07-04T09:30:00-04:00')`,
			v: fromISO.format("ddd, MMMM D YYYY · HH:mm Z"),
		},
		{
		...
		{
			m: `DateTime.fromMilliseconds(0, 'UTC')  // Unix epoch`,
			v: epoch.format("ddd, MMMM D YYYY · HH:mm:ss [UTC]"),
		},
		{
			m: `DateTime.fromUnix(1_700_000_000, 'America/Chicago')`,
			v: fromUnix.format("YYYY-MM-DD HH:mm:ss Z"),
		},
		{
		...
		{
			m: `DateTime.min(fromISO, fromObj)`,
			v: DateTime.min(fromISO, fromObj).format("YYYY-MM-DD"),
		},
		{
			m: `DateTime.max(fromISO, fromObj)`,
			v: DateTime.max(fromISO, fromObj).format("YYYY-MM-DD"),
		},
...
function FormatPlayground() {
	...
	return (
		<div>
			...
			<p style={{ color: "#334155", fontSize: "0.7rem", marginTop: "0.5rem" }}>
				Click a token to append it to the pattern
			</p>
		</div>
	);
}
...
function DateArithmetic() {
	...
	const ops: { label: string; dt: DateTime }[] = [
		...
		{
			label: ".setTimezone('America/New_York')",
			dt: base.setTimezone("America/New_York"),
		},
		{
			label: ".setTimezone('Asia/Tokyo', { keepLocalTime: true })",
			dt: base.setTimezone("Asia/Tokyo", { keepLocalTime: true }),
		},
		{
			label: ".set({ hour: 0, minute: 0, second: 0 })",
			dt: base.set({ hour: 0, minute: 0, second: 0 }),
		},
		...
		<div>
			...
			>
				<div>
					...
					>
						Search Timezones (568 IANA zones)
					</div>
					<input
						value={query}
			...
			>
				{[
					...
					{
						l: "Valid",
						v: info ? (info.valid ? "✅ Yes" : "❌ No") : "—",
						c: info?.valid ? "#10b981" : "#ef4444",
					},
					{ l: "Canonical", v: info?.canon ?? "—", c: "#f59e0b" },
					{
						l: "UTC Offset",
						v: info ? fmtOffset(info.offset) : "—",
						c: "#06b6d4",
					},
					{ l: "Abbreviation", v: info?.abbr ?? "—", c: "#a78bfa" },
					{
						l: "DST Active",
						v: info ? (info.isDST ? "🌞 Yes" : "❄️  No") : "—",
						c: info?.isDST ? "#fbbf24" : "#94a3b8",
					},
					{ l: "Current Time", v: info?.time ?? "—", c: "#34d399" },
					{
						l: "Offset (mins)",
						v: info ? String(info.offset) : "—",
						c: "#fb923c",
					},
				].map(({ l, v, c }) => (
		...
		<div>
			...
			>
				{[
					...
					{
						l: "humanize({ short: true })",
						v: dur.humanize({ short: true }),
						c: "#06b6d4",
					},
					{
						l: "humanize({ largest: 2 })",
						v: dur.humanize({ largest: 2 }),
						c: "#f59e0b",
					},
					{ l: "toISO()", v: dur.toISO(), c: "#a78bfa" },
					{
						l: "format('H[h] m[m] s[s]')",
						v: dur.format("H[h] m[m] s[s]"),
						c: "#ec4899",
					},
					...
					{
						l: "totalMilliseconds",
						v: dur.totalMilliseconds.toFixed(0),
						c: "#fbbf24",
					},
				...
				>
					...
					>
						{[
							...
							{
								k: "fromNow({ short:true })",
								v: dt.fromNow({ short: true }),
								c: "#06b6d4",
							},
...
function LocalTypes() {
	...
	const dateRows = [
		...
		{
			c: "month start: LocalDate.of(y, m, 1)",
			v: LocalDate.of(today.year, today.month, 1).toISO(),
		},
		{
			c: "month end:   LocalDate.of(y, m, daysInMonth)",
			v: LocalDate.of(today.year, today.month, today.daysInMonth).toISO(),
		},
		{
			c: "year start:  LocalDate.of(y, 1, 1)",
			v: LocalDate.of(today.year, 1, 1).toISO(),
		},
		{ c: ".add({ months: 1 })", v: today.add({ months: 1 }).toISO() },
		{ c: ".subtract({ days: 30 })", v: today.subtract({ days: 30 }).toISO() },
		{
			c: ".isBefore(today.add({ days: 1 }))",
			v: String(today.isBefore(today.add({ days: 1 }))),
		},
		{
			c: ".equals(LocalDate.today())",
			v: String(today.equals(LocalDate.today())),
		},
		{
			c: ".until(today.add({ days: 30 })) days",
			v: `${String(today.until(today.add({ days: 30 })))} days`,
		},
		{
			c: "LocalDate.of(2024, 2, 29).toISO()",
			v: LocalDate.of(2024, 2, 29).toISO(),
		},
		{
			c: "LocalDate.fromISO('2000-01-01').isLeapYear",
			v: String(LocalDate.fromISO("2000-01-01").isLeapYear),
		},
	];
	...
	const timeRows = [
		...
		{ c: '.format("HH:mm")', v: now.format("HH:mm") },
		{
			...
			v: `${now.hour} / ${now.minute} / ${now.second}`,
		},
		{ c: ".add({ hours: 6 })", v: now.add({ hours: 6 }).format("HH:mm:ss") },
		{
			c: ".subtract({ minutes: 45 })",
			v: now.subtract({ minutes: 45 }).format("HH:mm:ss"),
		},
		{ c: "LocalTime.of(9, 0)", v: LocalTime.of(9, 0).toISO() },
		{
			c: "LocalTime.of(23, 59, 59)",
			v: LocalTime.of(23, 59, 59).format("h:mm:ss A"),
		},
		{
			c: "LocalTime.fromISO('00:00')",
			v: LocalTime.fromISO("00:00").format("h:mm A"),
		},
		{
			c: "LocalTime.fromISO('13:30')",
			v: LocalTime.fromISO("13:30").format("h:mm A"),
		},
		{
			c: ".isBefore(LocalTime.of(23,59))",
			v: String(now.isBefore(LocalTime.of(23, 59))),
		},
		{
			c: "add wraps at midnight: LocalTime.of(22,0).add({hours:3})",
			v: LocalTime.of(22, 0).add({ hours: 3 }).format("HH:mm"),
		},
	];
					...
					>
						...
						>
							...
							<div style={{ minWidth: 0 }}>
								...
								<code style={{ color: "#334155", fontSize: "0.62rem" }}>
									{locale.name}
								</code>
							</div>
						</div>
...
function ComparisonsDemo() {
	...
	const rows: { expr: string; result: string }[] = [
		...
		{
			expr: "now.isSameOrBefore(future2h)",
			result: String(now.isSameOrBefore(future2h)),
		},
		{
			expr: "now.isSameOrAfter(past2h)",
			result: String(now.isSameOrAfter(past2h)),
		},
		{
			expr: "now.isBetween(rangeStart, rangeEnd)",
			result: String(now.isBetween(rangeStart, rangeEnd)),
		},
		{
			expr: "future2h.isBetween(rangeStart, rangeEnd)",
			result: String(future2h.isBetween(rangeStart, rangeEnd)),
		},
		{ expr: "now.isSame(now)", result: String(now.isSame(now)) },
		{
			expr: "now.isSame(sameDayPast, 'day')",
			result: String(now.isSame(sameDayPast, "day")),
		},
		{
			expr: "now.isSame(past2h, 'day')",
			result: String(now.isSame(past2h, "day")),
		},
		{ expr: "future2h.diff(past2h).humanize()", result: diff.humanize() },
		{
			expr: 'future2h.diff(past2h).totalHours + "h"',
			result: `${diff.totalHours.toFixed(2)}h`,
		},
		{
			expr: "DateTime.min(past2h, now, future2h).toISO()",
			result: DateTime.min(past2h, now, future2h).format("HH:mm:ss"),
		},
		{
			expr: "DateTime.max(past2h, now, future2h).toISO()",
			result: DateTime.max(past2h, now, future2h).format("HH:mm:ss"),
		},
	];
	...
	return (
		...
		>
			{(
				[
					{
						l: "a  = fromObject({ hours:3, minutes:30 })",
						v: a.humanize(),
						c: "#a78bfa",
					},
					{
						l: "b  = fromObject({ hours:1, minutes:45 })",
						v: b.humanize(),
						c: "#a78bfa",
					},
					{
						l: "c  = fromObject({ days:2, hours:4 })",
						v: c.humanize(),
						c: "#a78bfa",
					},
					...
					{
						l: "c.subtract(b).humanize()",
						v: c.subtract(b).humanize(),
						c: "#f59e0b",
					},
					{
						l: "a.subtract(b).humanize()",
						v: a.subtract(b).humanize(),
						c: "#f59e0b",
					},
					{
						l: "a.subtract(b).toISO()",
						v: a.subtract(b).toISO(),
						c: "#f59e0b",
					},
					{
						l: "a.add(b).totalDays",
						v: a.add(b).totalDays.toFixed(6),
						c: "#ec4899",
					},
					{ l: "c.totalDays", v: c.totalDays.toFixed(4), c: "#ec4899" },
					{
						l: "Duration.fromISO('-PT2H30M').isNegative()",
						v: String(neg.isNegative()),
						c: "#ef4444",
					},
					{ l: "neg.abs().humanize()", v: neg.abs().humanize(), c: "#34d399" },
					{ l: "neg.abs().toISO()", v: neg.abs().toISO(), c: "#34d399" },
					{
						l: "neg.abs().totalHours",
						v: neg.abs().totalHours.toFixed(4),
						c: "#60a5fa",
					},
					{
						l: "a.add(b).add(c).humanize()",
						v: a.add(b).add(c).humanize(),
						c: "#fb923c",
					},
				] as { l: string; v: string; c: string }[]
			).map(({ l, v, c }) => (
				<Chip key={l} label={l} value={v} color={c} />
			))}
		</div>
			...
			>
				<div>
					...
					<input
						type="datetime-local"
						value={dtAStr}
						onChange={(e) => setDtAStr(e.target.value)}
						style={inputStyle}
					/>
				</div>
				<div>
					...
					<input
						type="datetime-local"
						value={dtBStr}
						onChange={(e) => setDtBStr(e.target.value)}
						style={inputStyle}
					/>
				</div>
			</div>
			...
			>
				{(
					[
						...
						{
							l: "a.fromNow({ short: true })",
							v: dtA.fromNow({ short: true }),
							c: "#06b6d4",
						},
						{ l: "b.fromNow()", v: dtB.fromNow(), c: "#10b981" },
						{
							l: "b.fromNow({ short: true })",
							v: dtB.fromNow({ short: true }),
							c: "#06b6d4",
						},
						{ l: "a.toNow()", v: dtA.toNow(), c: "#f59e0b" },
						{
							l: "a.toNow({ short: true })",
							v: dtA.toNow({ short: true }),
							c: "#fb923c",
						},
						{ l: "b.toNow()", v: dtB.toNow(), c: "#f59e0b" },
						{
							l: "b.toNow({ short: true })",
							v: dtB.toNow({ short: true }),
							c: "#fb923c",
						},
						{ l: "a.to(b)", v: dtA.to(dtB), c: "#a78bfa" },
						{
							l: "a.to(b, { short: true })",
							v: dtA.to(dtB, { short: true }),
							c: "#8b5cf6",
						},
						{ l: "b.to(a)", v: dtB.to(dtA), c: "#a78bfa" },
						{
							l: "b.to(a, { short: true })",
							v: dtB.to(dtA, { short: true }),
							c: "#8b5cf6",
						},
					] as { l: string; v: string; c: string }[]
				).map(({ l, v, c }) => (
...
function DateTimeConversionsDemo() {
	...
	const base = DateTime.fromObject(
		{ year: yr, month: mo, day: dy, hour: hr, minute: mn, second: 0 },
			...
			>
				{(
					[
						...
						{
							l: "base.toDate().toISOString()",
							v: asJSDate.toISOString(),
							c: "#f59e0b",
						},
						{ l: "base.valueOf()", v: base.valueOf().toString(), c: "#fb923c" },
						{ l: "base.locale", v: JSON.stringify(base.locale), c: "#ec4899" },
						{
							l: "base.setYear(2030).year",
							v: String(base.setYear(2030).year),
							c: "#60a5fa",
						},
						{
							l: "base.setMonth(1).toISO().slice(0,10)",
							v: base.setMonth(1).toISO().slice(0, 10),
							c: "#818cf8",
						},
						{
							l: "base.setDay(1).toISO().slice(0,10)",
							v: base.setDay(1).toISO().slice(0, 10),
							c: "#818cf8",
						},
						{
							l: 'base.setTime(9, 0).format("HH:mm")',
							v: base.setTime(9, 0).format("HH:mm"),
							c: "#f472b6",
						},
						{
							l: "base.setYear(2000).setMonth(1).setDay(1)",
							v: base.setYear(2000).setMonth(1).setDay(1).toISO().slice(0, 10),
							c: "#fbbf24",
						},
					] as { l: string; v: string; c: string }[]
				).map(({ l, v, c }) => (
...
function LocalExtendedDemo() {
	...
	const timeFromObj = LocalTime.fromObject({
		hour: 14,
		minute: 30,
		second: 45,
	});
	...
	return (
		<div style={{ display: "grid", gap: "1.5rem" }}>
			<div>
				{subHead(
					"LocalDate — fromObject, fromDateTime, min, max, extended comparisons",
				)}
				<div
					style={{
						display: "grid",
				...
				>
					{(
						[
							{
								l: "LocalDate.fromObject({ year:2025, month:3, day:14 }).toISO()",
								v: fromObj.toISO(),
								c: "#f59e0b",
							},
							{
								l: "LocalDate.fromDateTime(DateTime.now()).toISO()",
								v: fromDt.toISO(),
								c: "#fb923c",
							},
							{
								l: "LocalDate.min(dateA, dateB, dateC).toISO()",
								v: LocalDate.min(dateA, dateB, dateC).toISO(),
								c: "#10b981",
							},
							{
								l: "LocalDate.max(dateA, dateB, dateC).toISO()",
								v: LocalDate.max(dateA, dateB, dateC).toISO(),
								c: "#34d399",
							},
							{
								l: "dateA.isAfter(today)",
								v: String(dateA.isAfter(today)),
								c: "#a78bfa",
							},
							{
								l: "dateC.isAfter(today)",
								v: String(dateC.isAfter(today)),
								c: "#a78bfa",
							},
							{
								l: "dateA.isSameOrBefore(today)",
								v: String(dateA.isSameOrBefore(today)),
								c: "#8b5cf6",
							},
							{
								l: "dateC.isSameOrAfter(today)",
								v: String(dateC.isSameOrAfter(today)),
								c: "#8b5cf6",
							},
							{
								l: "today.isBetween(dateA, dateC)",
								v: String(today.isBetween(dateA, dateC)),
								c: "#ec4899",
							},
							{
								l: "today.toDateTime('UTC', { hour:9 }).format('YYYY-MM-DD HH:mm')",
								v: today
									.toDateTime("UTC", { hour: 9 })
									.format("YYYY-MM-DD HH:mm"),
								c: "#06b6d4",
							},
							{
								l: "dateA.compareTo(dateB)",
								v: String(dateA.compareTo(dateB)),
								c: "#f59e0b",
							},
							{
								l: "dateC.compareTo(dateA)",
								v: String(dateC.compareTo(dateA)),
								c: "#fb923c",
							},
							{
								l: "today.isToday()",
								v: String(today.isToday()),
								c: "#10b981",
							},
							{
								l: 'today.isToday("UTC")',
								v: String(today.isToday("UTC")),
								c: "#34d399",
							},
							{
								l: "today.isWeekend()",
								v: String(today.isWeekend()),
								c: "#f472b6",
							},
							{
								l: "today.isWeekday()",
								v: String(today.isWeekday()),
								c: "#ec4899",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
			...
			<div>
				{subHead(
					"LocalTime — fromObject, fromDateTime, millisecondOfDay, extended comparisons",
				)}
				<div
					style={{
						display: "grid",
				...
				>
					{(
						[
							{
								l: "LocalTime.fromObject({ hour:14, minute:30, second:45 }).toISO()",
								v: timeFromObj.toISO(),
								c: "#f59e0b",
							},
							{
								l: "LocalTime.fromDateTime(DateTime.now()).toISO()",
								v: timeFromDt.toISO(),
								c: "#fb923c",
							},
							{
								l: "LocalTime.of(0,0,0).millisecondOfDay",
								v: String(midnight.millisecondOfDay),
								c: "#10b981",
							},
							{
								l: "LocalTime.of(12,0,0).millisecondOfDay",
								v: String(noon.millisecondOfDay),
								c: "#34d399",
							},
							{
								l: "LocalTime.now().millisecondOfDay",
								v: String(nowTime.millisecondOfDay),
								c: "#06b6d4",
							},
							{
								l: "timeFromObj.equals(LocalTime.of(14,30,45))",
								v: String(timeFromObj.equals(LocalTime.of(14, 30, 45))),
								c: "#a78bfa",
							},
							{
								l: "timeFromObj.isAfter(midnight)",
								v: String(timeFromObj.isAfter(midnight)),
								c: "#8b5cf6",
							},
							{
								l: "midnight.isSameOrBefore(noon)",
								v: String(midnight.isSameOrBefore(noon)),
								c: "#ec4899",
							},
							{
								l: "noon.isSameOrAfter(midnight)",
								v: String(noon.isSameOrAfter(midnight)),
								c: "#f472b6",
							},
							{
								l: "nowTime.isBetween(midnight, noon)",
								v: String(nowTime.isBetween(midnight, noon)),
								c: "#fb923c",
							},
							{
								l: "nowTime.compareTo(midnight)",
								v: String(nowTime.compareTo(midnight)),
								c: "#a78bfa",
							},
							{
								l: "midnight.compareTo(noon)",
								v: String(midnight.compareTo(noon)),
								c: "#8b5cf6",
							},
							{
								l: "noon.compareTo(noon)",
								v: String(noon.compareTo(noon)),
								c: "#6366f1",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
...
function DateTimeConvenienceDemo() {
	...
	return (
		<div style={{ display: "grid", gap: "1.75rem" }}>
			<div>
				...
				>
					{(
						[
							{
								l: "now.compareTo(tomorrow)",
								v: String(now.compareTo(tomorrow)),
								c: "#f59e0b",
							},
							{
								l: "tomorrow.compareTo(now)",
								v: String(tomorrow.compareTo(now)),
								c: "#fb923c",
							},
							{
								l: "now.compareTo(now)",
								v: String(now.compareTo(DateTime.now("America/New_York"))),
								c: "#a78bfa",
							},
							{
								l: "now.isBetween(yesterday, tomorrow)",
								v: String(now.isBetween(yesterday, tomorrow)),
								c: "#10b981",
							},
							{
								l: "future.isBetween(yesterday, tomorrow)",
								v: String(future.isBetween(yesterday, tomorrow)),
								c: "#34d399",
							},
							{
								l: "future.clamp(minDt, maxDt) — year",
								v: clamped.toISO().slice(0, 10),
								c: "#06b6d4",
							},
							{
								l: "now.weeksInYear",
								v: String(now.weeksInYear),
								c: "#60a5fa",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
			...
			<div>
				{subHead(
					"DateTime — calendar predicates (evaluated in instance timezone)",
				)}
				<div
					style={{
						display: "grid",
				...
				>
					{(
						[
							...
							{
								l: "now.isTomorrow()",
								v: String(now.isTomorrow()),
								c: "#8b5cf6",
							},
							{
								l: "now.isYesterday()",
								v: String(now.isYesterday()),
								c: "#7c3aed",
							},
							{
								l: "now.isWeekend()",
								v: String(now.isWeekend()),
								c: "#ec4899",
							},
							{
								l: "now.isWeekday()",
								v: String(now.isWeekday()),
								c: "#f472b6",
							},
							{
								l: "tomorrow.isToday()",
								v: String(tomorrow.isToday()),
								c: "#64748b",
							},
							{
								l: "tomorrow.isTomorrow()",
								v: String(tomorrow.isTomorrow()),
								c: "#10b981",
							},
							{
								l: "yesterday.isYesterday()",
								v: String(yesterday.isYesterday()),
								c: "#34d399",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
			...
			<div>
				...
				>
					{(
						[
							{
								l: "todayLD.compareTo(tomorrowLD)",
								v: String(todayLD.compareTo(tomorrowLD)),
								c: "#f59e0b",
							},
							{
								l: "tomorrowLD.compareTo(todayLD)",
								v: String(tomorrowLD.compareTo(todayLD)),
								c: "#fb923c",
							},
							{
								l: "todayLD.compareTo(todayLD)",
								v: String(todayLD.compareTo(todayLD)),
								c: "#a78bfa",
							},
							{
								l: "todayLD.isToday()",
								v: String(todayLD.isToday()),
								c: "#10b981",
							},
							{
								l: 'todayLD.isToday("UTC")',
								v: String(todayLD.isToday("UTC")),
								c: "#34d399",
							},
							{
								l: "pastLD.isToday()",
								v: String(pastLD.isToday()),
								c: "#64748b",
							},
							{
								l: "todayLD.isWeekend()",
								v: String(todayLD.isWeekend()),
								c: "#ec4899",
							},
							{
								l: "todayLD.isWeekday()",
								v: String(todayLD.isWeekday()),
								c: "#f472b6",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
			...
			<div>
				...
				>
					{(
						[
							{
								l: "nowTime.compareTo(morning)",
								v: String(nowTime.compareTo(morning)),
								c: "#f59e0b",
							},
							{
								l: "nowTime.compareTo(evening)",
								v: String(nowTime.compareTo(evening)),
								c: "#fb923c",
							},
							{
								l: "morning.compareTo(morning)",
								v: String(morning.compareTo(morning)),
								c: "#a78bfa",
							},
							{
								l: "nowTime.isBetween(morning, evening)",
								v: String(nowTime.isBetween(morning, evening)),
								c: "#10b981",
							},
							{
								l: "morning.isBetween(morning, evening)",
								v: String(morning.isBetween(morning, evening)),
								c: "#34d399",
							},
							{
								l: "evening.isBetween(morning, evening)",
								v: String(evening.isBetween(morning, evening)),
								c: "#06b6d4",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
			...
			<div>
				{subHead(
					"v0.3.0 — toLocaleDateString · toLocaleString · formatLocalized",
				)}
				<div
					style={{
						display: "grid",
				...
				>
					{(
						[
							{
								l: "dt.setLocale(de).toLocaleDateString()",
								v: dtDe.toLocaleDateString(),
								c: "#a78bfa",
							},
							{
								l: "dt.setLocale(de).toLocaleDateString({ dateStyle: 'full' })",
								v: dtDe.toLocaleDateString({ dateStyle: "full" }),
								c: "#8b5cf6",
							},
							{
								l: "dt.setLocale(ja).toLocaleDateString()",
								v: dtJa.toLocaleDateString(),
								c: "#06b6d4",
							},
							{
								l: "dt.setLocale(ar).toLocaleDateString()",
								v: dtAr.toLocaleDateString(),
								c: "#10b981",
							},
							{
								l: "dt.setLocale(fr).toLocaleDateString({ dateStyle: 'full' })",
								v: dtFr.toLocaleDateString({ dateStyle: "full" }),
								c: "#34d399",
							},
							{
								l: "dt.setLocale(hi).toLocaleDateString()",
								v: dtHi.toLocaleDateString(),
								c: "#f59e0b",
							},
							{
								l: "dt.setLocale(zh_Hans).toLocaleDateString()",
								v: dtZhHans.toLocaleDateString(),
								c: "#fb923c",
							},
							{
								l: "dt.setLocale(ar).toLocaleString({ timeStyle: 'short' })",
								v: dtAr.toLocaleString({ timeStyle: "short" }),
								c: "#ec4899",
							},
							{
								l: "dt.setLocale(de).toLocaleTimeString()",
								v: dtDe.toLocaleTimeString(),
								c: "#f472b6",
							},
							{
								l: "dt.setLocale(de).formatLocalized('long')",
								v: dtDe.formatLocalized("long"),
								c: "#60a5fa",
							},
							{
								l: "dt.setLocale(ja).formatLocalized('full')",
								v: dtJa.formatLocalized("full"),
								c: "#38bdf8",
							},
						] as { l: string; v: string; c: string }[]
					).map(({ l, v, c }) => (
...
function TzdataInfoDemo() {
	...
	return (
		<div style={{ display: "grid", gap: "1.25rem" }}>
			...
			>
				<Chip
					label="TIMEZONE_COUNT"
					value={String(TIMEZONE_COUNT)}
					color="#10b981"
				/>
				<Chip label="TZDATA_VERSION" value={TZDATA_VERSION} color="#a78bfa" />
				<Chip
					label="Timezone.listAll().length"
					value={String(Timezone.listAll().length)}
					color="#06b6d4"
				/>
				<Chip
					label="Object.keys(TIMEZONE_LINKS).length"
					value={String(Object.keys(TIMEZONE_LINKS).length)}
					color="#f59e0b"
				/>
			</div>
			<div>
	...
	return (
		<div>
			...
			>
				{(Object.keys(LOCALE_MAP) as LocaleKey[]).map((key) => (
					<button
						key={key}
						onClick={() => {
							setDefaultLocale(LOCALE_MAP[key]);
							setActive(key);
						}}
						style={{
			...
			>
				...
				/>
				<Chip
					label="now.toLocaleDateString({ dateStyle: 'full' })"
					value={now.toLocaleDateString({ dateStyle: "full" })}
					color="#8b5cf6"
				/>
				<Chip
					label="now.formatLocalized('long')"
					value={now.formatLocalized("long")}
					color="#6366f1"
				/>
				<Chip
				...
				<Chip
					label="now.subtract({ days:7 }).fromNow()"
					value={now.subtract({ days: 7 }).fromNow()}
					color="#fb923c"
				/>
				<Chip
					label="now.add({ months:3 }).fromNow()"
					value={now.add({ months: 3 }).fromNow()}
					color="#34d399"
				/>
			</div>
		</div>
			...
			>
				<div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem" }}>
							...
							<div>
								...
								<h1
									style={{
										...
										margin: "0 0 0.5rem",
										background:
											"linear-gradient(135deg,#e2e8f0 0%,#a78bfa 40%,#06b6d4 80%)",
								...
								>
									TypeScript-first date/time library with full IANA timezone
									support. Immutable API. Zero dependencies. &lt;20&nbsp;KB.
								</p>
					...
					>
						...
						<a
							href="https://www.npmjs.com/package/@yedoma-labs/tuuru-chrono-tz"
							target="_blank"
						...
						<span style={{ color: "#1e293b" }}>·</span>
						<a
							href="https://github.com/yedoma-labs/tuuru-chrono-tz"
							target="_blank"
							rel="noopener noreferrer"
							style={{
								color: "#64748b",
					...
					<Card>
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`import { DateTime, Timezone } from '@yedoma-labs/tuuru-chrono-tz';
...
Timezone.guessLocal();                          // 'America/New_York'`}
					...
					<Card>
						<SectionHeader
							emoji="🏗️"
							title="DateTime Construction Lab"
					...
					<Card>
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`const paris = DateTime.now('Europe/Paris');
...
paris.toUTC()                                            // convert to UTC`}
					...
					<Card>
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`Timezone.search('Pacific')            // ['Pacific/Auckland', 'Pacific/Fiji', ...]
...
Timezone.listAll().length              // 568+`}
					...
					<Card>
						...
						<div style={{ marginTop: "1rem" }}>
							<CodeBlock
								code={`import { Duration } from '@yedoma-labs/tuuru-chrono-tz';
...
diff.totalHours       // 123.0`}
					...
					<Card>
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`const past   = DateTime.now().subtract({ hours: 3 });
...
a.to(b, { short: true }) // "in 3d"`}
					...
					<Card>
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`import { LocalDate, LocalTime, de, ja, ar } from '@yedoma-labs/tuuru-chrono-tz';
...
today.isBefore(today.add({ days: 1 }))       // true`}
					...
					<Card>
						<SectionHeader
							emoji="🌐"
							title={`All ${ALL_LOCALES.length} Locales`}
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`import { DateTime, fr, ja, ar, zh, de } from '@yedoma-labs/tuuru-chrono-tz';
import { tk } from '@yedoma-labs/tuuru-chrono-tz/locales/tk';
...
neg.abs().totalHours      // 2.5`}
					...
					<Card>
						<SectionHeader
							emoji="🔗"
							title="to() · toNow() — Relative Between Two Points"
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`const past   = DateTime.now().subtract({ days: 3 });
...
past.to(future, { short: true })  // "in 10d"`}
					...
					<Card>
						<SectionHeader
							emoji="🔄"
							title="DateTime Conversions & Individual Setters"
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`const dt = DateTime.fromObject({ year:2025, month:6, day:15, hour:14, minute:30 }, 'America/New_York');
...
dt.setYear(2000).setMonth(1).setDay(1).toISO()  // "2000-01-01T14:30:00..."`}
					...
					<Card>
						<SectionHeader
							emoji="📅"
							title="LocalDate & LocalTime — Complete API"
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`import { LocalDate, LocalTime, DateTime } from '@yedoma-labs/tuuru-chrono-tz';
...
midnight.compareTo(noon)                   // -1 (before noon)`}
					...
					<Card>
						<SectionHeader
							emoji="🗄️"
							title="Timezone Database Constants"
...
Timezone.search('New')  // zones containing "New"`}
					...
					<Card>
						<SectionHeader
							emoji="🌍"
							title="Global Locale — setDefaultLocale · getDefaultLocale"
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`import { DateTime, setDefaultLocale, getDefaultLocale, fr, ja, ar } from '@yedoma-labs/tuuru-chrono-tz';
...
DateTime.now().setLocale(fr).formatLocalized('full')  // "lundi 15 juin 2026"`}
					...
					<Card>
						<SectionHeader
							emoji="✨"
							title="v0.2.0 / v0.3.0 — Convenience Methods & Locale Formatting"
						...
						<div style={{ marginTop: "1.5rem" }}>
							<CodeBlock
								code={`import { DateTime, LocalDate, LocalTime, de, ja, ar, fr } from '@yedoma-labs/tuuru-chrono-tz';
