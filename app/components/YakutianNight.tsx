...
export default function YakutianNight() {
	...
	useEffect(() => {
		...
		const doFreeze = (gx: number, gy: number) => {
			if (gx < 0 || gx >= GW || gy < 0 || gy >= GH || frozen[gy * GW + gx])
				return;
			frozen[gy * GW + gx] = 1;
			frozenCount++;
		...
		const bandY = (b: (typeof BANDS)[0], x: number) =>
			...
			Math.sin(x * b.fq * 2.62 + t * b.spd * 1.35 + b.ph + 1) *
				H *
				b.amp *
				0.42 +
			Math.sin(x * b.fq * 0.37 + t * b.spd * 0.27 + b.ph + 3) *
				H *
				b.amp *
				0.58;
		...
		const drawAurora = () => {
			...
			for (const b of BANDS) {
				...
				for (let i = 0; i < pts.length; i += 2) {
					const { x, y } = pts[i];
					const br =
						0.18 + 0.82 * Math.abs(Math.sin(i * 0.17 + t * b.spd * 1.9));
					const cH = H * 0.15 * br;
					if (cH < 3) continue;
