...
import { suruyZodAddProduct, suruyZodLogin } from "./zod-actions";

// Example 1: Full-featured contact form
function ContactFormWithSuruy() {
	const { state, action, pending, formRef } = useFormAction(
		suruySendContactMessage,
		{
			onSuccess: (data) => {
	...
	return (
		...
		>
			...
			>
				...
				with built-in zero-dependency schema validator
			</p>
			...
			<form ref={formRef} action={action} style={{ maxWidth: "600px" }}>
				...
				{state.success && state.data && (
					<div className="success">
						✅ Message sent successfully! (ID: {state.data.messageId})
					</div>
				)}
				{state.errors?._form && (
					<div className="error" role="alert">
	...
	return (
		...
		>
			...
			>
				...
				— no schema needed for simple forms
			</p>
			...
			<form ref={formRef} action={action} style={{ maxWidth: "600px" }}>
				...
				{state.success && state.data && (
					<div className="success">
						...
						{(state.data.fileSize / 1024).toFixed(2)} KB)
					</div>
				)}
			</form>
		</section>
	...
	return (
		...
		>
			...
			<form ref={formRef} action={action} style={{ maxWidth: "600px" }}>
				...
				{state.success && state.data && (
					<div className="success">
						✅ Account created! User ID: {state.data.userId}
					</div>
				)}
			</form>
		</section>
	...
	return (
		...
		>
			...
			<form ref={formRef} action={action} style={{ maxWidth: "600px" }}>
				...
				{state.success && state.data && (
					<div className="success">
						✅ Added {state.data.count} tag(s): {state.data.tags.join(", ")}
					</div>
				)}
			</form>
		</section>
	...
	return (
		...
		>
			...
			<form ref={formRef} action={action} style={{ maxWidth: "600px" }}>
				...
				{state.success && state.data && (
					<div className="success">
						✅ Logged in! Token: {state.data.token.substring(0, 20)}...
					</div>
				)}
			</form>
		</section>
...
function ZodProductExample() {
	...
	return (
		...
		>
			<h2 style={{ color: "#f1f5f9", marginBottom: "0.4rem" }}>
				✨ Advanced Zod (Refinements & Transforms)
			</h2>
			<p
				style={{
					color: "#94a3b8",
			...
			<form ref={formRef} action={action} style={{ maxWidth: "600px" }}>
				...
				{state.success && state.data && (
					<div className="success">
						✅ Product added! ${state.data.price.toFixed(2)} -{" "}
						{state.data.discount}% = ${state.data.finalPrice.toFixed(2)}
					</div>
				)}
			</form>
		</section>
	...
	return (
      ...
      `}</style>
			...
			>
				...
				<span style={{ color: "#1e293b" }}>·</span>
				<a
					href="https://github.com/yedoma-labs/suruy-form-actions"
					target="_blank"
					rel="noopener noreferrer"
					style={{
						color: "#64748b",
			...
			>
				...
				>
					...
					<strong style={{ color: "#fb923c" }}>React Server Actions</strong> and
					progressive enhancement — validation on the server, smooth UX on the
					client.
				</p>
				...
				>
					{[
						{
							icon: "🛡️",
							label: "Built-in Validator",
							desc: "Zero-dependency schema validation",
							color: "#fb923c",
						},
						{
							icon: "🔷",
							label: "Zod Support",
							desc: "Plug in your own schema library",
							color: "#6366f1",
						},
						{
							icon: "📁",
							label: "File Uploads",
							desc: "Custom file validation built-in",
							color: "#06b6d4",
						},
						{
							icon: "🔧",
							label: "Progressive",
							desc: "Works without JavaScript",
							color: "#34d399",
						},
						{
							icon: "⚡",
							label: "~3KB gzipped",
							desc: "Tiny bundle, zero runtime deps",
							color: "#f59e0b",
						},
			...
			>
				...
				<p style={{ color: "#94a3b8", margin: 0 }}>
					suruy-form-actions works seamlessly with Zod for advanced type-safe
					validation with refinements, transforms, and cross-field rules.
				</p>
			</div>
...
export const subscribeAction = createSimpleAction(async (formData) => {
  ...
  return { data: { email, message: 'Subscribed!' } }
			...
			>
				<h2 style={{ color: "#f1f5f9", marginBottom: "0.75rem" }}>
					⚖️ Compare with sir-forms
				</h2>
				<p style={{ color: "#94a3b8", marginBottom: "1.75rem" }}>
					See feature comparison and side-by-side code examples
				</p>
				<Link
					href="/comparison"
					style={{
					...
					{[
						{
							icon: "📦",
							title: "~3KB gzipped",
							desc: "Tiny bundle size with maximum functionality",
							color: "#fb923c",
						},
						{
							icon: "🔗",
							title: "Zero runtime deps",
							desc: "Built-in validator included, nothing extra",
							color: "#f59e0b",
						},
						{
							icon: "⚛️",
							title: "React 19 ready",
							desc: "Uses useActionState under the hood",
							color: "#06b6d4",
						},
						{
							icon: "🔧",
							title: "Progressive enhancement",
							desc: "Forms work without JavaScript",
							color: "#34d399",
						},
						{
							icon: "🔒",
						...
						},
						{
							icon: "🛡️",
							title: "Server-first",
							desc: "Validation runs on the server, secure by default",
							color: "#a78bfa",
						},
						{
							icon: "🎯",
							title: "Flexible",
							desc: "Use built-in validator or Zod, Valibot, etc.",
							color: "#ec4899",
						},
