...
const CONFIG_VERSION = (pkg.dependencies as Record<string, string>)[
	"@yedoma-labs/turar-config"
];

import { checkEnvHealth, generateSchemaDocs, loadConfig } from "./actions";
...
export default function ConfigPage() {
	...
	const [envHealth, setEnvHealth] = useState<
		EnvHealthData | { error: string } | null
	>(null);
	const [schemaDocs, setSchemaDocs] = useState<
		SchemaDocs | { error: string } | null
	>(null);
			...
			>
				<div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem" }}>
					...
					>
						<div
							style={{
								...
								bottom: 0,
								background:
									"radial-gradient(ellipse at 75% 50%,rgba(59,130,246,0.12) 0%,transparent 60%)",
								pointerEvents: "none",
							}}
						/>
						<div
							...
							<div>
								...
								<h1
									style={{
										...
										margin: "0 0 0.5rem",
										background:
											"linear-gradient(135deg,#e2e8f0,#93c5fd,#4ade80)",
								...
								>
									Multi-format config loading (JSON, YAML, TOML), hot reload,
									environment cascading, variable interpolation, Vault
									integration, and type-safe env validation.
								</p>
								<div
					...
					>
						...
						<span style={{ color: "#1e293b" }}>·</span>
						<a
							href="https://www.npmjs.com/package/@yedoma-labs/turar-config"
							target="_blank"
						...
						<span style={{ color: "#1e293b" }}>·</span>
						<a
							href="https://github.com/yedoma-labs/turar-config"
							target="_blank"
							rel="noopener noreferrer"
							style={{
								color: "#64748b",
					...
					<Card style={{ marginBottom: "1.5rem" }}>
						<SectionHeader
							...
							subtitle="Run live server-side demos — results appear below each button"
					...
					<Card style={{ marginBottom: "1.5rem" }}>
						<SectionHeader
							...
							subtitle="bylyt-env-guard can auto-generate markdown docs and .env.example from your schema — zero effort"
								...
								<>
									...
									>
										{(["env", "markdown"] as const).map((tab) => (
											...
											>
												{tab === "env" ? ".env.example" : "Markdown Docs"}
											</button>
										))}
									</div>
									<pre
										style={{
											background: "#020817",
											color: docsTab === "env" ? "#34d399" : "#93c5fd",
											padding: "1rem",
											borderRadius: "8px",
											fontSize: "0.72rem",
											overflowX: "auto",
					...
					<Card style={{ marginBottom: "1.5rem" }}>
						<SectionHeader
							...
							subtitle="turar-config supports JSON, YAML, and TOML — auto-detected by extension. Files cascade: default → {NODE_ENV}"
						...
						>
							{[
								{
									title: "config/default.json — Base configuration",
									color: "#3b82f6",
									code: `{
  ...
  "database": { "host": "localhost", "port": 5432, "name": "demo_db", "poolSize": 10 }
								...
								{
									title: "config/development.json — Dev overrides",
									color: "#f59e0b",
									code: `{
  ...
  "database": { "host": "localhost", "name": "demo_dev" }
								...
								{
									title: "config/production.json — Prod with interpolation",
									color: "#ef4444",
									code: `{
  ...
  "database": { "host": "\${DB_HOST}", "name": "\${DB_NAME}" }
					...
					<Card style={{ marginBottom: "1.5rem" }}>
						<SectionHeader
							emoji="💻"
							title="Code Examples"
							subtitle="How to wire up turar-config and bylyt-env-guard in a real Next.js project"
						...
						>
							<div>
								...
								>
									1. Define Configuration Schema (lib/config.ts)
								</h3>
								<CodeBlock
									code={`import { createConfigSync } from '@yedoma-labs/turar-config'
...
config.features_maxUploadSize // ✅ number`}
							...
							<div>
								...
								<CodeBlock
									code={`import { createEnv, eg } from '@yedoma-labs/bylyt-env-guard'
  ...
  return {
    ...
    analyticsEnabled:  config.features_enableAnalytics,
					...
					<Card style={{ marginBottom: "1.5rem" }}>
						<SectionHeader
							emoji="🔄"
							title="Environment Cascading"
							subtitle="Config loading order — later entries override earlier ones"
							...
							{[
								{
									step: "1",
									label: "default.json",
									desc: "Base configuration (always loaded)",
									color: "#94a3b8",
								},
								{
									step: "2",
									label: "{NODE_ENV}.json",
									desc: "Environment-specific overrides (development, staging, production)",
									color: "#3b82f6",
								},
								{
									step: "3",
									label: "Environment Variables",
									desc: "Variable interpolation with ${VAR_NAME}",
									color: "#10b981",
								},
								{
									step: "4",
									label: "Schema Defaults",
									desc: "Fallbacks from bylyt-env-guard schema definitions",
									color: "#a78bfa",
								},
								...
								>
									...
									<div>
										<code
											style={{ color, fontSize: "0.82rem", fontWeight: 700 }}
										>
											{label}
										</code>
										<p
					...
					<Card style={{ marginBottom: "1.5rem" }}>
						<SectionHeader
							...
							subtitle="Choosing between turar-config, bylyt-env-guard, or both"
							...
							{[
								{
									...
									color: "#3b82f6",
									bullets: [
										"Multi-environment file-based config",
										"JSON, YAML, TOML support",
										"Config cascading base → env",
										"Variable interpolation needed",
										"Vault secrets integration",
									],
								},
								{
									title: "bylyt-env-guard",
									color: "#10b981",
									bullets: [
										"Simple env var validation",
										"12-factor app principles",
										"Zero file-based config",
										"Minimal bundle size",
								...
								},
								{
									...
									bullets: [
										...
										"File defaults + env overrides",
										"Maximum flexibility + type safety",
										"Multiple deployment environments",
										"Config management is critical",
									],
								},
							...
							>
								...
								<tbody>
									{[
										...
										>
											<td
												style={{ padding: "0.6rem 0.75rem", color: "#94a3b8" }}
											>
												{feature}
											</td>
											<td
