...
function HybridProfileForm() {
	...
	const handleSubmit = useFormSubmit(
		adaptSuruyActionForSirForms(hybridUpdateProfile),
		{
			onSuccess: (data: any) => {
	...
	return (
		...
		>
			<h2 style={{ color: "#f1f5f9", marginBottom: "0.4rem" }}>
				👤 Hybrid Profile Form
			</h2>
			<p
				style={{
			...
			>
				<strong style={{ color: "#a78bfa" }}>Server:</strong> suruy schema
				validation &nbsp;|&nbsp;{" "}
				<strong style={{ color: "#6366f1" }}>Client:</strong> sir-forms state
				management
			</p>
			...
			<form onSubmit={handleSubmit}>
				...
				>
					<div>
						...
						{firstNameField.error && (
							<div className="error">{firstNameField.error}</div>
						)}
					</div>
					<div>
						...
						/>
						{lastNameField.error && (
							<div className="error">{lastNameField.error}</div>
						)}
					</div>
				</div>
				...
				{message && (
					<div style={msgStyle(message.startsWith("✅"))}>{message}</div>
				)}
			</form>
		</section>
	);
...
function HybridCheckoutForm() {
	...
	const handleSubmit = useFormSubmit(
		adaptSuruyActionForSirForms(hybridProcessCheckout),
		{
			onSuccess: (data: any) => {
				setMessage(
					`✅ Payment processed! Transaction ID: ${data.transactionId} | Amount: $${data.amount}`,
				);
			},
			onError: () => {
	...
	return (
		...
		>
			...
			>
				<strong style={{ color: "#a78bfa" }}>Server:</strong> Zod + suruy
				validation &nbsp;|&nbsp;{" "}
				<strong style={{ color: "#6366f1" }}>Client:</strong> sir-forms
				controlled inputs
			</p>
			...
			<form onSubmit={handleSubmit}>
				<div>
					...
					{cardNumberField.error && (
						<div className="error">{cardNumberField.error}</div>
					)}
				</div>
				...
				>
					<div>
						...
						<input
							...
							disabled={form.isSubmitting}
						/>
						{expiryMonthField.error && (
							<div className="error">{expiryMonthField.error}</div>
						)}
					</div>
					<div>
						...
						<input
							...
							disabled={form.isSubmitting}
						/>
						{expiryYearField.error && (
							<div className="error">{expiryYearField.error}</div>
						)}
					</div>
					<div>
				...
				<div>
					...
					/>
					{amountField.error && (
						<div className="error">{amountField.error}</div>
					)}
				</div>
...
function HybridMultiStepForm() {
	...
	const [message, setMessage] = useState<string | null>(null);
	const [step1Data, setStep1Data] = useState<Record<string, string> | null>(
		null,
	);
	const [step2Data, setStep2Data] = useState<Record<string, string> | null>(
		null,
	);
	...
	const handleStep1 = useFormSubmit(
		adaptSuruyActionForSirForms(hybridSubmitStep1),
		{
			onSuccess: () => {
				setStep1Data({
	...
	const handleStep2 = useFormSubmit(
		adaptSuruyActionForSirForms(hybridSubmitStep2),
		{
			onSuccess: () => {
				setStep2Data({
	...
	const handleFinalSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			const result =
				await adaptSuruyActionForSirForms(hybridFinalSubmit)(allData);

			if (result.success) {
				formLogger.info("Final submit success", { data: result.data });
				setMessage(
					`✅ Application submitted! ID: ${result.data.applicationId}`,
				);
				setTimeout(() => {
					setCurrentStep(1);
					setStep1Data(null);
	...
	return (
		...
		>
			...
			>
				<strong style={{ color: "#a78bfa" }}>Server:</strong> suruy validation
				per step &nbsp;|&nbsp;{" "}
				<strong style={{ color: "#6366f1" }}>Client:</strong> sir-forms
				maintains all state
			</p>
						...
						<div key={step} style={{ display: "flex", alignItems: "center" }}>
							<div
								style={{
									...
									justifyContent: "center",
									fontWeight: "bold",
									border: currentStep >= step ? "none" : "1px solid #334155",
									boxShadow:
										currentStep >= step
				...
				<form onSubmit={handleStep1}>
					...
					<div>
						...
						{companyNameField.error && (
							<div className="error">{companyNameField.error}</div>
						)}
					</div>
					<div>
						...
						{industryField.error && (
							<div className="error">{industryField.error}</div>
						)}
					</div>
					<div>
						...
						{employeeCountField.error && (
							<div className="error">{employeeCountField.error}</div>
						)}
					</div>
					<button type="submit" disabled={form.isSubmitting}>
				...
				<form onSubmit={handleStep2}>
					...
					<div>
						...
						<input
							...
							disabled={form.isSubmitting}
						/>
						{contactNameField.error && (
							<div className="error">{contactNameField.error}</div>
						)}
					</div>
					<div>
						...
						{contactEmailField.error && (
							<div className="error">{contactEmailField.error}</div>
						)}
					</div>
					<div>
						...
						<input
							...
							disabled={form.isSubmitting}
						/>
						{contactPhoneField.error && (
							<div className="error">{contactPhoneField.error}</div>
						)}
					</div>
			...
			{currentStep === 3 && step1Data && step2Data && (
				<form onSubmit={handleFinalSubmit}>
					...
					>
						...
						<p style={{ color: "#cbd5e1", margin: "0.3rem 0" }}>
							<strong style={{ color: "#94a3b8" }}>Company:</strong>{" "}
							{step1Data.companyName}
						</p>
						<p style={{ color: "#cbd5e1", margin: "0.3rem 0" }}>
							<strong style={{ color: "#94a3b8" }}>Industry:</strong>{" "}
							{step1Data.industry}
						</p>
						<p style={{ color: "#cbd5e1", margin: "0.3rem 0" }}>
							<strong style={{ color: "#94a3b8" }}>Employees:</strong>{" "}
							{step1Data.employeeCount}
						</p>
					</div>
					<div
						style={{
	...
	return (
      ...
      `}</style>
			...
			>
				...
				<a
					href="https://www.npmjs.com/package/@yedoma-labs/sir-forms"
					target="_blank"
				...
				<span style={{ color: "#1e293b" }}>·</span>
				<a
					href="https://www.npmjs.com/package/@yedoma-labs/suruy-form-actions"
					target="_blank"
					rel="noopener noreferrer"
					style={{
						color: "#fb923c",
			...
			>
				...
				<div
					style={{
						...
						height: "180px",
						background:
							"radial-gradient(circle, rgba(245,87,108,0.08) 0%, transparent 70%)",
						borderRadius: "50%",
						pointerEvents: "none",
					}}
				/>
				...
				>
					...
					<strong style={{ color: "#a78bfa" }}>suruy-form-actions</strong> for
					server validation +{" "}
					<strong style={{ color: "#6366f1" }}>sir-forms</strong> for client
					state.
				</p>
				...
				>
					{[
						{
							icon: "🛡️",
							label: "suruy-form-actions",
							desc: "Server-first validation, progressive enhancement, Zod support",
							color: "#a78bfa",
						},
						{
							icon: "⚡",
							label: "sir-forms",
							desc: "Controlled inputs, field-level state, smooth client UX",
							color: "#6366f1",
						},
						{
							icon: "✨",
							label: "Combined Power",
							desc: "Type-safe, robust validation + seamless user experience",
							color: "#34d399",
						},
			...
			<section style={{ marginTop: "3rem" }}>
				...
				>
					{[
						{
							icon: "🛡️",
							title: "Server-first validation",
							desc: "suruy handles validation on the server — security by default",
							color: "#a78bfa",
						},
						{
							icon: "⚡",
							title: "Rich client state",
							desc: "sir-forms gives you controlled components and field hooks",
							color: "#6366f1",
						},
						{
							icon: "🔧",
							title: "Progressive enhancement",
							desc: "Forms work without JS (from suruy)",
							color: "#06b6d4",
						},
						{
							icon: "🔒",
							title: "Type safety",
							desc: "Both libraries are fully typed, errors caught at compile time",
							color: "#f59e0b",
						},
						{
							icon: "🎯",
							title: "Flexible validation",
							desc: "Use suruy's built-in validator OR Zod",
							color: "#34d399",
						},
						{
							icon: "✨",
							title: "Smooth UX",
							desc: "Controlled inputs, loading states, optimistic updates",
							color: "#fb923c",
						},
				...
				>
					<h3 style={{ color: "#e2e8f0", marginBottom: "1.25rem" }}>
						Perfect For:
					</h3>
					<div
						style={{
