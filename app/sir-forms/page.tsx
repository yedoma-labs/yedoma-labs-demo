...
const SIRFORMS_VERSION = (pkg.dependencies as Record<string, string>)[
	...
	const handleSubmit = useFormSubmit(submitContactForm, {
		onSuccess: (data: any) => {
			...
			if (data?.name) {
				...
				setMessage({
					text: `Thanks ${data.name}! We'll be in touch soon.`,
					isError: false,
				});
			} else {
	...
	return (
		...
		>
			...
			>
				...
				from sir-forms v{SIRFORMS_VERSION}
			</p>
			<form onSubmit={handleSubmit}>
				<div>
					...
					/>
					{nameField.error && (
						<div id="sf-name-error" className="error" role="alert">
							{nameField.error}
						</div>
					)}
				</div>
				<div>
					...
					/>
					{emailField.error && (
						<div id="sf-email-error" className="error" role="alert">
							{emailField.error}
						</div>
					)}
				</div>
				<div>
					...
					<textarea
						...
						aria-describedby={
							messageField.error ? "sf-message-error" : undefined
						}
						disabled={form.isSubmitting}
						required
					/>
					{messageField.error && (
						<div id="sf-message-error" className="error" role="alert">
							{messageField.error}
						</div>
					)}
				</div>
				...
				{message && (
					<div style={msgStyle(!message.isError)}>{message.text}</div>
				)}
			</form>
		</section>
	);
...
function NewsletterForm() {
	...
	return (
		...
		>
			...
			>
				Minimal form — single field with sir-forms hooks, server-side email
				validation
			</p>
			<form onSubmit={handleSubmit} style={{ maxWidth: "480px" }}>
				<div>
					...
					<input
						...
						aria-invalid={emailField.error ? "true" : "false"}
						aria-describedby={
							emailField.error ? "newsletter-email-error" : undefined
						}
						disabled={form.isSubmitting}
					...
					/>
					{emailField.error && (
						<div id="newsletter-email-error" className="error" role="alert">
							{emailField.error}
						</div>
					)}
				</div>
...
function SubmissionTracker() {
	...
	return (
		...
		>
			...
			>
				<div>
					<h2 style={{ color: "#f1f5f9", marginBottom: "0.3rem" }}>
						📊 ichchi-state Tracker
					</h2>
	...
	return (
      ...
      `}</style>
			...
			>
				...
				<span style={{ color: "#1e293b" }}>·</span>
				<a
					href="https://github.com/yedoma-labs/sir-forms"
					target="_blank"
					rel="noopener noreferrer"
					style={{
						color: "#64748b",
			...
			>
				...
				>
					Type-safe React forms with{" "}
					<strong style={{ color: "#a78bfa" }}>FormProvider</strong>,
					field-level hooks, and server action validation — fully{" "}
					<strong style={{ color: "#6366f1" }}>React 19 compatible</strong>.
				</p>
					...
					{[
						{
							icon: "🔗",
							label: "FormProvider",
							desc: "Shared form context with initial values",
							color: "#a78bfa",
						},
						{
							icon: "🎯",
							label: "useField()",
							desc: "Field value, error, onChange per field",
							color: "#6366f1",
						},
						{
							icon: "✅",
							label: "useFormSubmit()",
							desc: "Submit handler with success/error callbacks",
							color: "#34d399",
						},
						{
							icon: "⚡",
							label: "React 19 ready",
							desc: "Compatible with latest concurrent features",
							color: "#f59e0b",
						},
						{
							icon: "🔒",
							label: "TypeScript first",
							desc: "Full type inference, zero runtime surprises",
							color: "#06b6d4",
						},
...
export async function myServerAction(prevState: unknown, formData: FormData) {
  ...
  return { data: { id: result.id, name } }
					...
					{
						title:
							"5. useServerAction — wrap any server action with loading state",
				...
				{[
					{
						href: "/comparison",
						icon: "⚖️",
						title: "Compare with suruy-form-actions",
						desc: "See feature comparison and side-by-side code",
						color: "#f59e0b",
					},
					{
						href: "/hybrid",
						icon: "🔄",
						title: "Hybrid Demo",
						desc: "Combine sir-forms with suruy-form-actions server validation",
						color: "#34d399",
					},
					{
						href: "/features",
						icon: "🚀",
						title: "ichchi-state Features",
						desc: "Time-travel debug, computed values, cross-tab sync",
						color: "#a78bfa",
					},
