...
import { addTodoOptimistic, incrementLikes } from "./actions";
...
function LikeButton() {
	...
	return (
		...
		>
			...
			>
				<span style={{ color: "#a78bfa", fontSize: "0.7rem", fontWeight: 700 }}>
					useServerAction
				</span>
			</div>
			<h3
			...
			{error && (
				<div
					style={{ color: "#ef4444", marginTop: "0.75rem", fontSize: "0.8rem" }}
				>
					Error: {error}
				</div>
			)}
...
function ComputedStats() {
	...
	return (
		...
		>
			...
			>
				<span style={{ color: "#34d399", fontSize: "0.7rem", fontWeight: 700 }}>
					computed()
				</span>
			</div>
			<h3
			...
			>
				Auto-updating derived state from formStore — updates whenever the store
				changes.
			</p>
			{!mounted ? (
					...
					{[
						{
							...
							value: stats.hasSubmissions ? "✅ Yes" : "❌ No",
							color: stats.hasSubmissions ? "#10b981" : "#475569",
						},
						{
							label: "Active user (3+)",
							value: stats.isActive ? "🔥 Yes" : "❌ No",
							color: stats.isActive ? "#f59e0b" : "#475569",
						},
						{
...
function TimeTravelDebugger() {
	...
	const createSnapshot = () => {
		...
		snapshots.create(
			id,
			`Manual snapshot at ${new Date().toLocaleTimeString()}`,
		);
	...
	const restore = (id: string) => {
		if (snapshots.restore(id))
			notify(`Restored to: ${id.replace("snapshot-", "")}`);
	};
...
function OptimisticTodoList() {
	...
	return (
		...
		>
			...
			>
				<span style={{ color: "#c4b5fd", fontSize: "0.7rem", fontWeight: 700 }}>
					optimistic()
				</span>
			</div>
			<h3
...
function HistoryBranchingDemo() {
	...
	const storeRef = useRef<ReturnType<
		typeof createStore<{ count: number }>
	> | null>(null);
	const branchingRef = useRef<ReturnType<typeof createHistoryBranching> | null>(
		null,
	);
	...
	const inc = () =>
		storeRef.current?.setState((s: any) => ({ count: s.count + 1 }));
	const dec = () =>
		storeRef.current?.setState((s: any) => ({ count: s.count - 1 }));
	const back = () => {
		branchingRef.current?.back();
		setCount(storeRef.current?.getState().count ?? 0);
		setTree(branchingRef.current?.visualize() ?? "");
	};
	const fwd = () => {
		branchingRef.current?.forward();
		setCount(storeRef.current?.getState().count ?? 0);
		setTree(branchingRef.current?.visualize() ?? "");
	};
	const branch = () => {
		...
		setNewBranchName("");
		setBranches(
			branchingRef.current?.getBranches().map((b: any) => b.name) ?? [],
		);
			...
			>
				{branches.map((br) => (
					<button
						...
						type="button"
						onClick={() => {
							branchingRef.current?.checkout(br);
							setCurrentBranch(br);
							setCount(storeRef.current?.getState().count ?? 0);
							setTree(branchingRef.current?.visualize() ?? "");
						}}
...
function FieldSubscriptionDemo() {
	...
	const [values, setValues] = useState<FState>({
		name: "",
		score: 0,
		active: false,
	});
	const [log, setLog] = useState<
		{ field: string; value: string; ts: string }[]
	>([]);
	const [mounted, setMounted] = useState(false);
	...
	useEffect(() => {
		...
		const addLog = (field: string, value: unknown) =>
			setLog((l) =>
				[
					{ field, value: String(value), ts: new Date().toLocaleTimeString() },
					...l,
				].slice(0, 10),
			);

		const u1 = subscribeToField(store, "name", (v: string) => {
			addLog("name", v);
			setValues((s) => ({ ...s, name: v }));
		});
		const u2 = subscribeToField(store, "score", (v: number) => {
			addLog("score", v);
			setValues((s) => ({ ...s, score: v }));
		});
		const u3 = subscribeToField(store, "active", (v: boolean) => {
			addLog("active", v);
			setValues((s) => ({ ...s, active: v }));
		});
		return () => {
			u1();
			u2();
	...
	const set = (patch: Partial<FState>) =>
		storeRef.current?.setState((s: FState) => ({ ...s, ...patch }));
	...
	return (
		...
		>
			...
			>
				<span style={{ color: "#22d3ee", fontSize: "0.7rem", fontWeight: 700 }}>
					subscribeToField()
				</span>
			</div>
			<h3
					...
					>
						{log.length === 0 ? (
							<p style={{ color: "#334155", fontSize: "0.72rem" }}>
								Edit a field above to see targeted notifications…
							</p>
						) : (
							log.map((e, i) => (
								<div
								...
								>
									<span
										style={{
											color: fieldColors[e.field] ?? "#94a3b8",
											fontSize: "0.65rem",
											fontFamily: "monospace",
											width: 36,
											flexShrink: 0,
...
function BatchUpdateDemo() {
	...
	const normalStoreRef = useRef<ReturnType<typeof createStore<BState>> | null>(
		null,
	);
	const batchedStoreRef = useRef<ReturnType<typeof createStore<BState>> | null>(
		null,
	);
	...
	const [batchedState, setBatchedState] = useState<BState>({
		x: 0,
		y: 0,
		z: 0,
	});
	...
	useEffect(() => {
		...
		setMounted(true);
		const u1 = ns.subscribe((s: BState) => {
			setNormalState(s);
			setNormalNotifs((n) => n + 1);
		});
		const u2 = bs.subscribe((s: BState) => {
			setBatchedState(s);
			setBatchedNotifs((n) => n + 1);
		});
		return () => {
			u1();
			u2();
	...
	const barColor = (v: number, max: number) =>
		`${Math.min(100, max > 0 ? (v / max) * 100 : 0)}%`;
							...
							{(["x", "y", "z"] as const).map((k) => (
								...
								>
									...
									>
										<div
											style={{
												width: barColor(normalState[k], maxVal),
												height: "100%",
												background: "#ef4444",
							...
							{(["x", "y", "z"] as const).map((k) => (
								...
								>
									...
									>
										<div
											style={{
												width: barColor(batchedState[k], maxVal),
												height: "100%",
												background: "#10b981",
												transition: "width 0.2s",
											}}
					...
					{(normalNotifs > 0 || batchedNotifs > 0) && (
						...
						>
							...
							<strong style={{ color: "#fbbf24" }}>
								{normalNotifs} vs {batchedNotifs}
							</strong>{" "}
							notifications for the same state change
							{normalNotifs > 0 && batchedNotifs > 0 && (
								<>
									{" "}
									— batching saved{" "}
									<strong style={{ color: "#10b981" }}>
										{Math.round((1 - batchedNotifs / normalNotifs) * 100)}% of
										renders
									</strong>
								</>
							)}
			...
			>
				<div style={{ maxWidth: 1200, margin: "0 auto", padding: "2rem" }}>
							...
							<div>
								...
								<h1
									style={{
										...
										margin: "0 0 0.5rem",
										background:
											"linear-gradient(135deg,#e2e8f0,#c4b5fd,#818cf8)",
								...
								>
									Computed values, time-travel debugging, optimistic updates,
									and cross-tab sync — all powered by ichchi-state.
								</p>
								<div
					...
					>
						...
						<a
							href="https://www.npmjs.com/package/@yedoma-labs/ichchi-state"
							target="_blank"
						...
						<span style={{ color: "#1e293b" }}>·</span>
						<a
							href="https://github.com/yedoma-labs/ichchi-state"
							target="_blank"
							rel="noopener noreferrer"
							style={{
								color: "#64748b",
					...
					>
						...
						>
							Open this page in multiple tabs and submit the contact form on the
							home page. All tabs sync automatically via localStorage
...
branching.visualize()           // ASCII tree of history`,
							...
							{
								title:
									"5. createBatcher() + subscribeToField() — fine-grained control",
...
subscribeToField(store, 'x', (value) => console.log('x changed to', value))`,
							...
							{
								title:
									"6. createOptimisticUpdates() — instant UI with rollback",
